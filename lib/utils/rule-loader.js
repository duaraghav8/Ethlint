...
let rules = {'solium-all': require("../../config/rulesets/solium-all.js"), 'solium-recommended': require("../../config/rulesets/solium-recommended.js")}
let rulesIndex = require('../rules/ruleIndex.js')
let plugins = {'solium-plugin-security': require('solium-plugin-security')}
...
function resolveUpstream(upstream) {
    ...
    if (coreRulesetRegExp.test(upstream)) {
        try {
            var name = "solium-" + upstream.split(":")[1]
            return (rules[name] || require("../../config/rulesets/solium-" + name)).rules;
        } catch (e) {
...
function load(listOfRules) {
    ...
    listOfRules.forEach(function(name) {
        ...
        try {
            ruleDefs [name] = rulesIndex[name] || require(ruleFile);
        } catch (e) {
