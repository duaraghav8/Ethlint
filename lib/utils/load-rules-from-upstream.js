...
module.exports = function (upstream, SOLIUM_CORE_RULES_DIR, rulesToLoad) {
	...
	// Only solium:recommended is supported for now. In future, we can expand to URLs and registries.
	switch (upstream) {
		case 'solium:recommended':
			RULES_DIR = SOLIUM_CORE_RULES_DIR;
			break;
		default:
			throw new Error ('\"' + upstream + '\": Unknown rule repository. Solium only supports \"solium:recommended\" for now.')
	}
