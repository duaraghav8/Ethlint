...
/**
 * Get the Nth parent node of the specified node. 0th parent is the node itself. 1st parent
 * is an immediate parent of a node.
 * @param {Object} node The AST Node to retrieve the parent of
 * @param {Integer} n number of a parent node starting from zero
 * @returns {Object} nodeParent Nth parent node of the given node
 */
exports.getNthParent = function(node, n) {
    if (n < 0) {
        throw new Error("Parent number should be greater of equal to zero");
    }
    let parent = node;
    for (let i = 0; i < n; i++) {
        parent = exports.getParent(parent);
    }
    throwIfInvalidNode(parent);
    return parent;
};
