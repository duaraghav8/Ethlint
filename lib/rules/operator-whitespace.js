    ...
    create: function(context) {
        ...
        function inspectAssignmentExpression(emitted) {
            ...
            (!validationRegexp.test(charsAfterLeftNode)) && context.report({
                ...
                fix(fixer) {
                    const fixedLeft = context.getSourceCode().getText(node.left).trim() + " ";
                    const fixedRight = " " + context.getSourceCode().getText(node.right).trim();
                    return [fixer.replaceTextRange(
                        [node.left.start, node.right.end],
                        fixedLeft + node.operator + fixedRight
                    )];
                }
            });
        }
            ...
            for (let i = 2; i < code.length; i++) {
                if (code[i] === "=") {
                    (!/^[^\/\s] $/.test(code.slice(i-2, i))) && context.report({
                        ...
                        fix(fixer){
                            let fixedMultipleSpace = context.getSourceCode().getText(node).replace(/  +=/g, " =");
                            let noSpacesMatch = fixedMultipleSpace.match(/[A-Za-z]+=/g);
                            if (noSpacesMatch) {
                                noSpacesFix = noSpacesMatch[0].replace(/=/g, " =");
                                let fixedAll = fixedMultipleSpace.replace(/[A-Za-z]+=/g, noSpacesFix);
                                return [fixer.replaceText(node, fixedAll)];
                            }
                            else return [fixer.replaceText(node, fixedMultipleSpace)];
                        }
                    });
                    ...
                    (!/^ [^\/\s]$/.test(code.slice(i+1, i+3))) && context.report({
                        ...
                        fix(fixer){
                            let fixedMultipleSpace = context.getSourceCode().getText(node).replace(/= +/g, "= ");
                            let noSpacesMatch = fixedMultipleSpace.match(/=[A-Za-z0-9]+/g);
                            if (noSpacesMatch) {
                                let noSpacesFix = noSpacesMatch[0].replace(/=/g, "= ");
                                let fixedAll = fixedMultipleSpace.replace(/=[A-Za-z0-9]+/g, noSpacesFix);
                                return [fixer.replaceText(node, fixedAll)];
                            }
                            else return [fixer.replaceText(node, fixedMultipleSpace)];
                        }
