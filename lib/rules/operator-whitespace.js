            ...
            for (let i = 2; i < code.length; i++) {
                if (code[i] === "=") {
                    (!/^[^\/\s] $/.test(code.slice(i-2, i))) && context.report({
                        ...
                        fix(fixer){
                            // TODO: trim whitespaces around '=' char
                            const fixedLeft = code.slice(0, i) + " " + code.slice(i, code.length);
                            return [fixer.replaceText(node, fixedLeft)];
                        }
                    });
                    ...
                    (!/^ [^\/\s]$/.test(code.slice(i+1, i+3))) && context.report({
                        ...
                        fix(fixer){
                            // TODO: trim whitespaces around '=' char
                            const fixedRight = code.slice(0, i+1) + " " + code.slice(i+1, code.length);
                            return [fixer.replaceText(node, fixedRight)];
                        }
                    });
                }
            }
