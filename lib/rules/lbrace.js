...
module.exports = {
    ...
    create: function(context) {
        ...
        function inspectFunctionDeclaration(emitted) {
            ...
            if (node.returnParams) {
                let lastNodeBeforeBody = node.returnParams.slice(-1) [0];

                if (sourceCode.getLine(node.body) !== (sourceCode.getEndingLine(lastNodeBeforeBody) + 1)) {
                    context.report({
                        node: node.body,
                        message: (node.is_abstract ? "Fallback function" : `'${node.name}'`) +
                            ": Opening brace must be on a new line immediately after the last modifier."
                    });
                }
            ...
            if (node.modifiers) {
                ...
                let stringBetwLastModifAndLBrace,
