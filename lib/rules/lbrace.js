        ...
        function inspectFunctionDeclaration(emitted) {
            ...
            if (declarationEndingLine === declarationStartingLine) {
                ...
                if (bodyStartingLine !== declarationEndingLine) {
                    ...
                    (!/[^\s\/] /.test(charsBeforeLBrace)) && context.report({
                        node: node.body,
                        message: "Function body opening brace must be preceded by only a single space."
                    });
                }
            ...
            if (node.modifiers) {
                ...
                (!/^[ \t]*\n[ \t]*$/.test(stringBetwLastModifAndLBrace)) && context.report({
                    node: node.body,
                    message: "Opening brace must be on a new line immediately after the last modifier."
                });
