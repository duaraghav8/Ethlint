	...
	verify: function (context) {
		...
		context.on ('IfStatement', function (emitted) {
			...
			if (
				...
				context.report ({
					node: node,
					message: '\'else if (..) {..}\' Clause must be on its own line immediately after the previous one.'
				});
			}
			...
			} else if (!/^[^\s\/] $/.test (sourceCode.getPrevChars (node.consequent, 2))) {
				context.report ({
					node: node.consequent,
			...
			if (node.alternate) {
				...
				if (node.alternate.type === 'EmptyStatement') {
					...
					if (!/^[^\s\/] $/.test (sourceCode.getPrevChars (node.alternate, 2))) {
						context.report ({
							node: node.consequent,
							message: 'There should be only a single space and no comments between \'else\' and opening brace.'
						});
					}
