	...
	verify: function (context) {
		...
		context.on ('VariableDeclaration', function (emitted) {
			...
			for (var i = 2; i < code.length; i++) {
				if (code [i] === '=') {
					(!/^\w $/.test (code.slice (i-2, i))) && context.report ({
						node: node,
						message: '\'=\' must be preceeded by exactly a single space.' 
					});

					(!/^ \w$/.test (code.slice (i+1, i+3))) && context.report ({
						node: node,
						message: '\'=\' must be succeeded by exactly a single space.' 
					});
				}
			}
		});
