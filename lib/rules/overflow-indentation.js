...
module.exports = {
	...
	create: function (context) {
		...
		var MAX_IN_SINGLE_LINE = context.options ? context.options [0] : 3;
		...
		function inspectArrayExpression (emitted) {
			...
			if (sourceCode.getLine (node) === endingLineNum) {
				if (elements.length > MAX_IN_SINGLE_LINE) {
					context.report ({
						node: node,
						message: 'In case of more than ' + MAX_IN_SINGLE_LINE + ' elements, array expression needs to be spread over multiple lines with 1 element per line.'
					});
				}
				return;
			}
