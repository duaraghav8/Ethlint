    ...
    create: function(context) {
        ...
        function inspectAssemblyBlock(emitted) {
            ...
            for (let line = sourceCode.getLine(node) + 1; line < endingLineNum; ++line) {
                // maybe, a little slow
                const assemblyIndent = getIndentString(BASE_INDENTATION_STYLE, currentIndentLevel + subLevel);
                const assemblyIndentDesc = getIndentDescription(BASE_INDENTATION_STYLE, currentIndentLevel + subLevel);
                const indentRegExp = new RegExp("^" + assemblyIndent + "[^\\s(\/\*)]");
    
                let codeLineText = sourceCode.getTextOnLine(line);
                !indentRegExp.test(codeLineText) && context.report({
                    node: node,
                    message: `Only use indent of ${assemblyIndentDesc}.`
                });

                let codeLineWithoutStrings = codeLineText.replace(/"([^"]|\\")*"|'([^']|\\')*'/g, "");
                subLevel += (codeLineWithoutStrings.match(/{/g) || []).length -
                            (codeLineWithoutStrings.match(/}/g) || []).length;
            }
        }
