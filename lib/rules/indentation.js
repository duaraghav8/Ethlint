		...
		context.on ('StructDeclaration', function (emitted) {
			...
			function inspectStructAttr (structIndent, attr) {
				...
				!indentRegExp.test (attrLineText) && context.report ({
					node: attr,
					message: 'Incorrect Indentation: Make sure you use exacly \"' + structIndent + '\" for indentation on this line and don\'t preceed the declaration by comment(s).'
				});
			}
		...
		context.on ('FunctionDeclaration', function (emitted) {
			...
			function inspectParam (paramIndent, param) {
				...
				!indentRegExp.test (paramLineText) && context.report ({
					node: param,
					message: 'Incorrect Indentation: Make sure you use exacly \"' + paramIndent + '\" for indentation on this line and don\'t preceed the declaration by comment(s).'
				});
			}
			...
			var lastArgLine = sourceCode.getEndingLine (params.slice (-1) [0]);

			if (sourceCode.getLine (node) === lastArgLine) {
		...
		context.on ('CallExpression', function (emitted) {
			...
			function inspectArgument (argIndent, argument) {
				...
				!indentRegExp.test (argLineText) && context.report ({
					node: argument,
					message: 'Incorrect Indentation: Make sure you use exacly \"' + argIndent + '\" for indentation on this line and don\'t preceed the declaration by comment(s).'
				});
			}
