    ...
    create: function(context) {
        ...
        if (context.options) {
            if (context.options [0] === "tab") {
                ...
                const spCount = context.options [0];
        ...
        const BASE_INDENTATION_STYLE_REGEXP_GLOBAL = new RegExp(BASE_INDENTATION_STYLE, "g");


        const sourceCode = context.getSourceCode();
        const topLevelDeclarations = ["ContractStatement", "LibraryStatement"];

        //Ensures NO indentation exists before top-level declarations (contract, library)
        ...
        function inspectArrayExpression(emitted) {
            ...
            const endingLineNum = sourceCode.getEndingLine(node);
            let arrayExpressionLineText, currentIndent, currentIndentLevel;
