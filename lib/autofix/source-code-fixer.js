...
module.exports = {
	...
	applyFixes: function (sourceCode, errorMessages) {
		var fixedSourcedCode = '', fixes = [], fixesApplied = [], remainingMessages = [];
		...
		function attemptFix (fix) {
			...
			fixedSourcedCode += sourceCode.slice (Math.max (0, cursor), Math.max (0, start));
			fixedSourcedCode += fix.text;
			return true;
		}


		errorMessages.forEach (function (msg) {
			if (Object.hasOwnProperty (msg, 'fix')) {
		...
		fixedSourcedCode += sourceCode.slice (Math.max (0, cursor));
		remainingMessages.sort (compareMessagesByLocation);

		return {
			fixesApplied: fixesApplied,
			fixedSourcedCode: fixedSourcedCode,
