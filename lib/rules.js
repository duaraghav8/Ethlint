...
module.exports = {
	...
	load: function (config, noReset) {
		...
		if (config.rules && Object.keys (config.rules).length) {
			...
			// If it doesn't, we import the rules turned on in config.rules from "solium:recommended" ruleset.
			if (config.extends) {
				...
				Object.assign (rules, loadRulesFromUpstream ('solium:recommended', RULES_DIR, Object.keys (config.rules)));
			}
	...
	getRuleSeverity: function getRuleSeverity (ruleConfig) {
		if (Number.isInteger (ruleConfig)) {
			if (ruleConfig >= 0 && ruleConfig <= 2) {
				return ruleConfig;
			}

			throw new Error (ruleConfig + ' is not a valid rule severity value. Only 0, 1 or 2 can be used.');
		}
		...
		if (typeof ruleConfig === 'string') {
			...
			throw new Error ('\"' + ruleConfig + '\" is not a valid severity value. Only \"off\", \"warning\" or \"error\" can be used.');
		}

		// If value is a valid array, recursively call getRuleSeverity() passing in the first item of it.
		if (Array.isArray (ruleConfig)) {
			if (!ruleConfig.length) {
				throw new Error ('An empty array cannot be passed for a rule configuration. The first argument must specify the severity.');
			}
