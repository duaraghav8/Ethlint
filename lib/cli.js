...
function lint(userConfig, input, ignore, errorReporter) {
    ...
    if (input.file) {
        ...
        if (!fsUtils.isFile(input.file)) {
            errorReporter.reportFatal("Input is not a valid file");
        ...
        if (!fsUtils.isDirectory(input.dir)) {
            errorReporter.reportFatal("Input is not a valid directory");
            process.exit(errorCodes.INVALID_PARAMS);
        }
...
function createCliOptions(cliObject) {
    cliObject
        ...
        .option("-c, --config [path]", "Specify the path to .soliumrc configuration file");
}
