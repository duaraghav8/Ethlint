...
function displayDiff(fileName, sourceCode, fixedSourceCode) {
    let diff = jsdiff.diffLines(sourceCode, fixedSourceCode);
    console.log((`Diff for: ${fileName}`).cyan);
    diff.forEach(function(part){
        process.stderr.write(getDiffLine(part));
    });
}

function getDiffLine(diffPart) {
    if (diffPart.added) {
        return `${("+").green} ${diffPart.value["green"]}`;
    } else if (diffPart.removed) {
        return `${("-").red} ${diffPart.value["red"]}`;
    }

    return diffPart.value["grey"];
}
