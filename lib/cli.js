...
var CWD = process.cwd (),
	...
	CONFIG_FILENAME_ABSOLUTE = path.join (CWD, CONFIG_FILENAME);
...
/**
 * Synchronously write the passed configuration to the file whose absolute path is CONFIG_FILENAME_ABSOLUTE
 ...
 */
function createOptions (cliObject) {
	cliObject
		.version (version)
		...
		.option ('-s, --sync', 'Make sure that all Solium rules enabled by default are specified in your ' + CONFIG_FILENAME);
}
...
function execute (programArgs) {
	...
	try {
		userConfig = require (CONFIG_FILENAME_ABSOLUTE);
	} catch (e) {
		console.log (
