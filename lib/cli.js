 ...
 */
function lint (userConfig, fileOrDir) {
	var filesToLint = [];

	//If filename is provided, lint it. Otherwise, lint over current directory & sub-directories
	if (fileOrDir.file) {
		filesToLint.push (fileOrDir.file);
	} else {
		traverse (fileOrDir.dir || CWD, filesToLint);
	}
...
module.exports = {
	...
	execute: function execute (programArgs) {
		...
		if (cli.init) {
			...
			lint (userConfig, {	file: cli.file,	dir: cli.dir });
			...
			if (cli.hot) {
				...
				spy.on ('change', function () {
					lint (userConfig, {	file: cli.file,	dir: cli.dir });	//lint on subsequent changes (hot)
