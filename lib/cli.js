...
function execute(programArgs) {
    if (Math.random() < VERSION_CHECK_CHANCE) {
        checkVersionAndRunSolium(programArgs);
    } else {
        runSolium(programArgs);
    }
}

function checkVersionAndRunSolium(programArgs) {
    let soliumNpmUrl = "https://registry.npmjs.org/solium?version=latest";

    axios.get(soliumNpmUrl)
        .then(function(response) {
            const latestVersion = response.data.version;
            if (semver.gt(version, latestVersion)) {
                process.stdout.write(`You are running a dev version ${version}, laterst version is ${latestVersion}`.yellow);
            } else if (semver.lt(version, latestVersion)) {
                process.stdout.write(`You are runninng an outdated version ${version}, latest version is ${latestVersion}`.yellow);
            } else {
                process.stdout.write(`You are runninng the latest version: ${version}`.green);
            }
            runSolium(programArgs);
        })
        .catch(function(error) {
            process.stdout.write(`Failed to get solium version: ${error}`);
            runSolium(programArgs);
        });
}

function runSolium(programArgs) {
