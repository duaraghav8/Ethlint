...
module.exports = (function () {
	...
	Solium.lint = function lint (sourceCode, config, noReset) {
		...
		if (configInspector.isFormatDeprecated (currentConfig)) {
			// If user is still using a deprecated config format, notify.
			...
			currentConfig.rules = rules.loadUsingDeprecatedConfigFormat (currentConfig.rules, currentConfig ['custom-rules-filename']);
		} else {
		...
		try {
			...
			throw new Error ('An error occured while parsing the source code:\n' + e.stack);
		}
	...
	Solium.reportInternal = function reportInternal (issue) {
		...
		messages.push (Object.assign (issue, { internal: true }));
	};
