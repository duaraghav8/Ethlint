	...
	Solium.lint = function lint (sourceCode, config, noReset) {
		...
		if (configInspector.isFormatDeprecated (currentConfig)) {
			...
			crf && Solium.reportInternal ({
				type: 'warning',
				message: '[DEPRECATED] Attribute "custom-rules-filename" is now deprecated. Rules from ' + crf + ' were not loaded. Plugins are supported v1 onward. Please check documentation.'
			});
